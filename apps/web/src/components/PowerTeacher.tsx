...
function colorName(color: StoneColor, locale: Locale): string {
  return authored(locale, color === 'black' ? 'Black' : 'White', color === 'black' ? '黑棋' : '白棋', color === 'black' ? '黒' : '白')
}
...
function selectedSteps(
  ...
  if (!preview || !samePoint(preview.point, selected)) {
    return [
      { label: 'Play', sentences: [{ source: 'Lesson guidance', text: authored(locale, `Inspect ${colorName(toPlay, locale)} at ${coordinate}; nothing has been placed.`, `检查${colorName(toPlay, locale)}下在 ${coordinate} 的情况；尚未落子。`, `${colorName(toPlay, locale)}が ${coordinate} に打つ場合を調べます。まだ着手していません。`) }] },
  ...
  if (!preview.legal) {
    return [
      { label: 'Play', sentences: [{ source: 'Exact rules', text: authored(locale, `${coordinate} is not legal now.`, `${coordinate} 当前不合法。`, `${coordinate} には現在打てません。`) }] },
  ...
  if (preview.captures.length) exactChanges.push(authored(locale, `captures ${preview.captures.length} stone${preview.captures.length === 1 ? '' : 's'}`, `提掉 ${preview.captures.length} 枚棋子`, `${preview.captures.length} 子を取り`))
  if (preview.resulting_liberties != null) exactChanges.push(authored(locale, `leaves the new string ${preview.resulting_liberties} libert${preview.resulting_liberties === 1 ? 'y' : 'ies'}`, `让新棋串有 ${preview.resulting_liberties} 口气`, `新しい一団に ${preview.resulting_liberties} ダメを残す`))
  ...
  const fewestAdjacentLiberties = adjacentGroups.length
    ...
    : null
  return [
    { label: 'Play', sentences: [{ source: 'Exact rules', text: authored(locale, `${colorName(toPlay, locale)} can legally play ${coordinate}; the stone is still only a preview.`, `${colorName(toPlay, locale)}可以合法下在 ${coordinate}；棋子仍只是预览。`, `${colorName(toPlay, locale)}は ${coordinate} に合法に打てますが、まだプレビューです。`) }] },
    ...
    { label: 'Changes', sentences: [{ source: 'Exact rules', text: exactChanges.length ? authored(locale, `It ${exactChanges.join(' and ')}.`, `这手棋会${exactChanges.join('，并')}。`, `この手は${exactChanges.join('、')}。`) : authored(locale, 'No capture was reported; compare the resulting liberties and connections.', '没有报告提子；请比较着后的气与连接。', '取りは報告されていません。着手後のダメと連絡を比較します。') }] },
    ...
    { label: 'Then check', sentences: [{ source: 'Teacher interpretation', text: fewestAdjacentLiberties != null ? authored(locale, `The adjacent group with the fewest current liberties has ${fewestAdjacentLiberties}; check whether it is actually urgent before comparing the preview’s ${preview.resulting_liberties ?? 'unreported'} liberties.`, `相邻棋块中最少的当前气数是 ${fewestAdjacentLiberties}；先判断它是否真的紧急，再比较预览结果的${preview.resulting_liberties ?? '未报告'}口气。`, `隣接する一団の現在の最小ダメ数は ${fewestAdjacentLiberties}。本当に急場かを確かめてから、プレビュー結果の ${preview.resulting_liberties ?? '未報告'} ダメと比較します。`) : authored(locale, 'Compare a concrete local reply before a broad follow-up.', '先比较一个具体的局部应手，再考虑宽广的后续。', '広い続きの前に、具体的な局所応手を比較します。') }] },
    { label: 'Principle', sentences: [{ source: 'Lesson guidance', text: authored(locale, 'A group in atari is urgent; otherwise compare the concrete cost of answering now or later.', '被叫吃的棋块很紧急；否则，请比较现在应对与稍后应对的具体代价。', 'アタリの一団は急場です。それ以外は、今応じる場合と後で応じる場合の具体的な代償を比較します。') }] },
  ]
}
...
function currentSteps(
  ...
  return [
    ...
    { label: 'Changes', sentences: [{ source: 'Exact rules', text: lastGroup ? authored(locale, `The last move’s connected string has ${lastGroup.liberties.length} distinct liberties.`, `最后一手的连通棋串有 ${lastGroup.liberties.length} 口不重复的气。`, `最終手の連結した一団には ${lastGroup.liberties.length} 個の異なるダメがあります。`) : authored(locale, 'No candidate-specific rules result is available yet.', '尚无针对候选着的规则结果。', '候補手固有のルール結果はまだありません。') }] },
...
export function PowerTeacher({
  ...
  const evidenceMode = size < 9
    ? authored(locale, `Authored ${size}×${size} view · no KataGo claim`, `人工编写的 ${size}×${size} 视图 · 不声称有 KataGo 依据`, `教材用の ${size}×${size} 表示 · KataGo の主張なし`)
    : candidate
