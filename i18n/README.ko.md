...
Path of Influence는 Go/Weiqi를 위한 로컬 우선 교육 애플리케이션입니다. 짧은 5×5 및 7×7 수업에서 완전한 9×9 게임으로 이동하며, 설명이 포함된 동반자, 제한된 플레이어 에이전트, 내레이터 에이전트 극장, 재생 가능한 연대기를 제공합니다. 정확한 규칙은 모든 분석이나 언어 모델 제공자가 오프라인일 때에도 권위적입니다.
...
- 일반 게임은 선언된 중국 지역 규칙을 사용하며, 훈련 변형은 레이블이 붙어 있습니다.
...
이것은 활성 개발 중인 생산 교육 애플리케이션이며, 일회용 보드 게임 데모가 아닙니다. 소스는 MIT 라이센스이며, KataGo 및 다운로드된 네트워크는 상류 라이센스를 유지하며 여기에는 커밋되지 않습니다. 현재 기본값은 9×9이며, 13×13 및 19×19는 별도로 예산이 책정된 계획으로 남아 있습니다.
